/**
 * FileQueueStorage — a cross-process queue storage backed by the local filesystem.
 *
 * Jobs are stored as individual JSON files in a data directory:
 *   <dataDir>/queued/<id>.json   — waiting to be picked up
 *   <dataDir>/leased/<id>.json   — currently being processed
 *   <dataDir>/dead/<id>.json     — permanently failed (dead-letter)
 *
 * `fs.renameSync` provides atomic "claim" semantics on POSIX filesystems —
 * only one process/worker wins the race to move a file from queued → leased.
 * This makes the storage safe for multiple worker processes polling the same dir.
 */
...
export class FileQueueStorage implements QueueStorage {
  ...
  async dequeue(options: LeaseOptions): Promise<StoredJob[]> {
    ...
    // 1. Claim fresh jobs from the queued directory
    ...
    for (const file of queuedFiles) {
      ...
      try {
        ...
        // Atomic claim: rename fails if another process already moved it
        ...
        // Another worker claimed this file — skip it
      }
    }

    // 2. Re-claim expired leases (visibility timeout exceeded)
    if (results.length < batchSize) {
      ...
      for (const file of leasedFiles) {
        ...
        try {
          ...
          if (queue && job.queue !== queue) continue;
          if ((job._leaseExpiry ?? 0) > now) continue; // still valid
          ...
          // File disappeared — already processed by another worker
        }
      }
    }
  ...
  async moveToDeadLetter(job: StoredJob): Promise<void> {
    ...
    try {
      ...
      // Fallback: write directly to dead-letter
      const dead: StoredJob = { ...job, state: 'failed', updatedAt: Date.now() };
      fs.writeFileSync(dst, JSON.stringify(dead));
    }
  }
