...
export function SilencedJobsPage({ queues }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Failed to load silenced jobs');
    } finally {
      setLoading(false);
    }
  }, [queueFilter]);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Silenced Jobs</h2>
          <p className="text-slate-400 text-sm mt-1">Completed jobs hidden from the main completed list by dashboard rules.</p>
        </div>
        <button
          onClick={() => {
            void load();
